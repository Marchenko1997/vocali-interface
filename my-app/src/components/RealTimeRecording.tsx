...
const RealTimeRecording: React.FC<RealTimeRecordingProps> = ({
  ...
  const cleanTranscriptText = (text: string) => {
    ...
    const finalText =
      uniqueSentences.join(". ") + (uniqueSentences.length > 0 ? "." : "");
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-sm text-yellow-800">
            Speechmatics API key not configured. Please add your actual API key
            to VITE_SPEECHMATICS_API_KEY in your .env file.
          </span>
        </div>
      </div>
    );
...
const RecordingInterface: React.FC<RecordingInterfaceProps> = ({
  ...
  const [localAccumulatedTranscript, setLocalAccumulatedTranscript] = useState(
    accumulatedTranscript,
  );
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (data.message === "RecognitionStarted") {
            ...
            if (data.metadata && data.metadata.transcript) {
              ...
              setLocalAccumulatedTranscript((prevText) => {
                ...
                if (!prevText || newPartial.length > prevText.length) {
                  console.log(
                    "Updating accumulated transcript with longer partial:",
                    newPartial,
                  );
                  return newPartial;
                }
            ...
            setTimeout(() => {
              console.log(
                "Final transcript after utterance end:",
                localAccumulatedTranscript,
              );
            }, 1000);
    ...
    <div className="bg-white rounded-2xl shadow-lg p-6">
      <div className="text-center mb-6">
        ...
        <p className="text-gray-600">
          Record audio directly from your microphone and get real-time
          transcription
        </p>
      </div>
